...
def vaxijen_to_df(vaxijen_txt):
    ...
    #     text = file.read()
    with open(vaxijen_txt, "r", encoding="utf-8") as file:
        text = file.read()
...
st.markdown(
    "<h1 style='text-align: center;'>AutoEpiCollect 2.0 Demo</h1>",
    unsafe_allow_html=True
)

st.markdown(
    """
    ##### This is the demo workspace where users can get familiar with AutoEpiCollect's functionality using sample files and a dummy dataset. Below, you will find the normal user interface, but with these sample files pre-loaded in: 
    1. "variant_list.csv": This contains sample Partek Flow output to start the vaccine design process with.
    2. "vaxijen_test_mhc_I.txt": This file has the antigenicity outputs collected from VaxiJen.
    3. "aggregated_mhc_I.xlsx": This file has the output MHC I epitopes from Step 1, which is further analyzed with the antigenicity data.
    """
)
...
with tab2:
    ...
    # )
    antigenicity_files = "vaxijen_test_mhc_I.txt"
    antigenicity_df_mhci = vaxijen_to_df(antigenicity_files)
    ...
    mhc_xlsx_files = ["aggregated_mhc_I.xlsx"]
